...
class Olama_School_Admin
{
    ...
    public function render_curriculum_management_page()
    {
        ...
        $tabs_config = array(
            ...
            'timeline' => array('label' => Olama_School_Helpers::translate('Timeline'), 'cap' => 'olama_manage_curriculum_timeline'),
                                                    ...
                                                    <tr valign="top">
                                                        ...
                                                        <td>
                                                            <input type="text" name="olama_school_settings[ramadan_start]"
                                                                ...
                                                                <?php echo !$is_admin ? 'disabled' : ''; ?> />
                                                            <p class="description">
                                                                <?php _e('Dates when the Ramadan schedule will be active.', 'olama-school'); ?></p>
                                                        </td>
                                                    </tr>
                                                    <tr valign="top">
                                                        <th scope="row">
    ...
    public function render_weekly_plan_management_page()
    {
        ...
        if (Olama_School_Permissions::can('olama_approve_plans')) {
            $review_count = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}olama_plans WHERE status IN ('submitted', 'needs_edit')");
        } else {
            $review_count = $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM {$wpdb->prefix}olama_plans WHERE status = 'needs_edit' AND teacher_id = %d",
                get_current_user_id()
            ));
        }
